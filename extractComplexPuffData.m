function [rawResults, normalizedMeanResults, normalizedMedianResults] = extractComplexPuffData(dataStruct) 
    rawResults = [];
    ...
    for i=1:numel(dataStruct)
        ...
        for j=puffs
            ...
            datum = [i tracks(j).lifetime_s trapz(tracks(j).Ac) tracks(j).start ((time_to_peak*0.1) - 0.1) tau_one_half*0.1 plateau*0.1];
            rR = [rR; datum];
            nmeanR = [nmeanR; datum];
